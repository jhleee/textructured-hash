...
# 2. 페어 생성
python scripts/generate_pairs.py --input data/synthetic.jsonl \
...
```bash
python scripts/implement_baselines.py
python scripts/evaluate.py --model baseline_rp --data data/test.jsonl
python scripts/evaluate.py --model baseline_simhash --data data/test.jsonl
python scripts/evaluate.py --model baseline_minhash --data data/test.jsonl
python scripts/evaluate.py --model baseline_tfidf --data data/test.jsonl
```
...
```python
# 1. Profiling
python -m cProfile -o profile.stats scripts/evaluate.py --model best_model
...
# 4. 최적화 후 재평가
python scripts/benchmark_efficiency.py --model optimized_model
```
...
## 10. Git Workflow & Experiment Tracking

### 10.1 커밋 규칙

**각 실험마다 별도의 커밋을 생성하세요.**

```bash
# 예시: 베이스라인 구현
git commit -m "feat: implement baseline RandomProjection encoder"
git commit -m "feat: implement baseline SimHash encoder"
git commit -m "feat: implement baseline MinHash encoder"
git commit -m "feat: implement baseline TF-IDF+SVD encoder"

# 예시: 실험 결과
git commit -m "experiment: evaluate RandomProjection (AUC=0.82)"
git commit -m "experiment: evaluate SimHash (AUC=0.78)"

# 예시: 제안 알고리즘
git commit -m "feat: implement MultiScale encoder"
git commit -m "experiment: evaluate MultiScale v1 (AUC=0.89)"
git commit -m "feat: improve MultiScale with better normalization"
git commit -m "experiment: evaluate MultiScale v2 (AUC=0.91)"
```

### 10.2 실험 리포트 저장

각 실험마다 `results/` 디렉토리에 리포트를 저장하세요:
...
```bash
# 1. 알고리즘 구현
python scripts/implement_algorithm.py --model new_algorithm
git add src/encoders/new_algorithm.py
git commit -m "feat: implement NewAlgorithm encoder"

# 2. 실험 실행 및 리포트 생성
python scripts/evaluate.py --model new_algorithm --output results/new_algorithm/
git add results/new_algorithm/
git commit -m "experiment: evaluate NewAlgorithm (AUC=X.XX, F1=X.XX)"

# 3. 개선 사항 반영
# ... 코드 수정 ...
git add src/encoders/new_algorithm.py
git commit -m "feat: improve NewAlgorithm with XYZ optimization"

# 4. 재실험
python scripts/evaluate.py --model new_algorithm --output results/new_algorithm_v2/
git add results/new_algorithm_v2/
git commit -m "experiment: evaluate NewAlgorithm v2 (AUC=X.XX, F1=X.XX)"
```

### 10.4 커밋 메시지 컨벤션

- `feat:` - 새로운 알고리즘, 기능 구현
- `experiment:` - 실험 실행 및 결과
- `fix:` - 버그 수정
- `refactor:` - 코드 리팩토링
- `docs:` - 문서 업데이트
- `perf:` - 성능 최적화
...
- [ ] 4개 베이스라인 구현 및 평가 완료
  - [ ] 각 베이스라인마다 별도 커밋 생성
  - [ ] 각 베이스라인마다 results/ 디렉토리에 리포트 저장
- [ ] 최소 2개 제안 알고리즘 구현 및 평가 완료
  - [ ] 각 알고리즘마다 별도 커밋 생성
  - [ ] 각 실험마다 results/ 디렉토리에 리포트 저장
- [ ] Ablation study 완료
- [ ] 효율성 벤치마크 완료
- [ ] 결과 보고서 작성 완료
- [ ] 최고 성능 알고리즘 코드 정리 완료
